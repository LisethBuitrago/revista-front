...
export class Usuario implements OnInit {
  ...
  alternarCifradoDesdeBackend(): void {
    ...
    if (this.publicacionSeleccionada) {
      if (!this.todoDesencriptado) {
        this.encriptadorService.desencriptar(this.publicacionSeleccionada.titulo).subscribe((res: any) => this.publicacionSeleccionada.titulo = res);
        this.encriptadorService.desencriptar(this.publicacionSeleccionada.contenido).subscribe((res: any) => this.publicacionSeleccionada.contenido = res);
      } else {
        this.encriptadorService.encriptar(this.publicacionSeleccionada.titulo).subscribe((res: any) => this.publicacionSeleccionada.titulo = res);
        this.encriptadorService.encriptar(this.publicacionSeleccionada.contenido).subscribe((res: any) => this.publicacionSeleccionada.contenido = res);
      }
    }
  ...
  filtrarPorCategoria(event: Event) {
    ...
    if (categoria === 'TODAS') {
      ...
      this.tarjetasFiltradas = this.tarjetas.filter(item => item.tipo.toUpperCase() === categoria);
    }
  }
